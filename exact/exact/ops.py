...
def quantize_and_pack(data, bits, mn, mx):
    if config.simulate:
        ...
        if config.use_optimal_lo:
            dim = data.size(-1)
            a, b, c, d, f = 0.83162565, 0.02993653, 116.05528511, 0.00001225, -0.00171617
            config.lo = a + b * np.log(dim * c) + d * dim + f * dim**0.5
        lo, hi = config.lo, 3 - config.lo
        output = pack_func(data, mn, mx, scale.to(data.dtype), bits, config.stochastic, lo, hi)
        if config.swap:
            output = swap_to_cpu(output)
    return output, scale, (lo, hi)


def dequantize_and_unpack(data, shape, bits, scale, mn, lo, hi):
    if config.simulate:
        ...
        data = unpack_func(data, bits, scale, mn, N, num_features, lo, hi)
    return data
...
def closest_multiple(col_size, multiple):
    remainder = col_size % multiple
    if remainder > multiple / 2:
        return col_size + (multiple - remainder)
    else:
        return col_size - remainder


def quantize_activation(input, scheme):
    #input = expand_middle(input)
    # col_size is actually col_size multiplier
    col_size = config.col_size * input.size(-1)
    col_size = np.clip(col_size, input.size(-1), 1024)
    col_size = closest_multiple(col_size, input.size(-1))
    
    # Calculate the size of the reshaped tensor
    elements = input.numel()
    remaining_elements = elements % col_size
    main_part_rows = (elements - remaining_elements) // input.size(1)

    orig_feat_dim = input.size(-1)
    input2 = input[main_part_rows:].clone()

    #input = input.narrow(0, 0, main_part_rows).view(-1, col_size)
    ...
    q_input, q_scale, (lo, hi) = quantize_and_pack(input_groups, q_bits, q_min, mx)
    if input.dtype == torch.float32:
        return q_input, q_bits, q_scale.to(torch.bfloat16), q_min.to(torch.bfloat16), input2, input.shape, orig_feat_dim, (lo, hi)
    else:
        return q_input, q_bits, q_scale, q_min, input2, input.shape, orig_feat_dim, (lo, hi)
...
def dequantize_activation(quantized, q_input_shape):
    ...
    q_input, q_bits, q_scale, q_min, input2, q_input_shape, orig_feat_dim, (lo, hi) = quantized
    if q_scale.dtype == torch.bfloat16:
        q_scale = q_scale.to(torch.float32)
        q_min = q_min.to(torch.float32)
    input = dequantize_and_unpack(q_input, q_input_shape, q_bits, q_scale, q_min, lo, hi)     

    input = input.view(-1, orig_feat_dim)
...
class qspmm_mean(Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        if value_requires_grad:
            ...
            if quantized[2] is None:
                other = torch.tensor([1.], dtype=torch.float, device='cuda')
            else:
