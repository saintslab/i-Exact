...
Tensor pack_single_precision_cuda(
    Tensor data, Tensor min, Tensor max, Tensor scale, int bits, bool stochastic, float lo, float hi);
Tensor unpack_single_precision_cuda(
    Tensor data, int bits, Tensor scale, Tensor min, int64_t N, int group_size, float lo, float hi);
...
Tensor pack_single_precision(Tensor data,
  ...
  return pack_single_precision_cuda(data, min, max, scale, bits, stochastic, lo, hi);
}
