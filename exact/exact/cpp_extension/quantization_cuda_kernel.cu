...
__global__ void pack_single_precision_kernel(int32_t bits,
  ...
  for (int ni = 0; ni < work_per_thread; ni++) {
    ...
    scalar_t x = (data[id] - min[n]) * scale[n];

    if (lo != 1.0) {
      if (x <= lo) {
      x = x / lo; // Scale x into the range (0, 1)
      }
      else if (x >= hi) {
        x = 2.0f + (x - hi) / (3.0f - hi); // Scale x into the range (2, 3)
      }
      else { // lo < x < hi
        x = 1.0f + (x - lo) / (hi - lo); // Scale x into the range (1, 2)
      }
    }
    const int32_t val = __float2int_rn(fmax(x + noise - 0.5f, 0.0f));
    ...
    if (N % work_per_thread == 0) {
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "pack_single_precision", ([&] {
        pack_single_precision_kernel<scalar_t, false><<<block_dim, thread_dim>>>(
          ...
          rng_engine_inputs,
          N, group_size, block_idx_y_base, lo, hi);
      }));
    } else {
      AT_DISPATCH_FLOATING_TYPES_AND_HALF(data.scalar_type(), "pack_single_precision", ([&] {
...
__global__ void unpack_single_precision_kernel(int32_t bits,
  ...
  for(int64_t no=blockIdx.x; no < num_blocks; no += gridDim.x){
    ...
    for (int ni = 0; ni < work_per_thread; ni++) {
      ...
      const scalar_t val_unscaled = ((scalar_t)val);
      
      if (lo != 1.0) {
        if (val_unscaled == 1.0) {
          // transform val_unscaled from (0, 1) to lo
          unpacked[id] = lo / scale[n] + min[n];
        } else if (val_unscaled == 2.0) {
          // transform val_unscaled from (2, 3) to hi
          unpacked[id] = hi / scale[n] + min[n];
        } else {
          unpacked[id] = val_unscaled / scale[n] + min[n];
        }
      } else {
