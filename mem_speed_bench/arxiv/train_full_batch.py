...
parser.add_argument('--test_co2', action='store_true', help='whether to test the carbon footprint')
parser.add_argument('--col_size', type=int, default=0)
parser.add_argument('--lo', type=float, default=1.0)
parser.add_argument('--use_optimal_lo', action='store_true')
...
@torch.no_grad()
def call_model(model, data, n_classes, use_labels=False, amp_mode=False):
    model.eval()
    if use_labels:
...
def main():
    ...
    if args.col_size is not None:
        config.col_size = args.col_size

    if args.lo is not None:
        config.lo = args.lo

    if args.use_optimal_lo:
        config.use_optimal_lo = True
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + model_config['epochs']):
            ...
            if result[1] > best_val:
                #print(f'{epoch}: New best model found')
                best_model = deepcopy(model)
                best_val = result[1]
        
            if epoch == model_config['epochs']:
                config.last_epoch = True
                call_model(best_model, data, num_classes, use_labels, args.amp)
                config.last_epoch = False
